 ...
 * 通知视图对象（站内消息中心列表）。
 *
 * actor 为触发互动的人，work 为被互动的作品；作品已硬删时 workTitle 回退为“作品已删除”。
...
public class NotificationVO {
    ...
    @Schema(description = "接收者（作品作者）用户 ID", example = "u-1a2b3c4d5e6f")
    private String recipientId;

    @Schema(description = "触发者（点赞/收藏的人）用户 ID", example = "u-1a2b3c4d5e6f")
    ...
    @Schema(description = "被互动的作品 ID", example = "w-1a2b3c4d5e6f")
    private String workId;

    @Schema(description = "作品标题；作品已删除时为“作品已删除”", example = "周末的湖边日落")
    private String workTitle;

    @Schema(description = "作品封面 URL", example = "/uploads/xxx.jpg", nullable = true)
    private String workCoverUrl;

    @Schema(description = "通知类型：like=点赞 / favorite=收藏", example = "like")
    private String type;
